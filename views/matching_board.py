...
def generate_weekly_matrix_html(df_source, dates_for_week, days_of_week_map, teacher_branch_map=None, all_branch_teachers=None, is_summer_mode=False, is_print_mode=False):
    ...
    🌟 日付ごとに枠数を動的に可変させることで、不要な余白を完全に削ぎ落とします。
    """
    ...
    # ヘッダー1行目（日付・曜日）：その日のスロット数に応じて colspan を動的に変更
    ...
    # データ行
    for t in teachers if (teachers := all_target_teachers) else []:
...
def render_matching_page():
    ...
    <style>
        ...
        .print-optimized-table { width:100%; border-collapse: collapse; min-width: 1600px; background-color: #ffffff; color: #333333; font-family: sans-serif; font-size: 12px; }
        ...
        .slot-header { background-color: #fcfcfc; font-size: 11px; font-weight: bold; color: #555; width: 45px; }
        ...
        .data-cell { vertical-align: top; min-width: 45px; background-color: #ffffff; }
    ...
    # 🌟 各大きなタブ（通常 / 夏期）の中の処理をループで共通化しつつ、フラグで制御
    for tab_idx, is_summer in enumerate([False, True]):
        with period_tabs[tab_idx]:
            ...
            # デフォルト表示期間の最適化（夏期なら7月後半、通常なら今日を基準に）
            ...
            with tab_create:
                ...
                if st.button(btn_label, type="primary", key=f"gen_btn_{is_summer}", use_container_width=True):
                    with st.spinner("アルゴリズムを実行中..."):
                        ...
                        # 🌟 講習契約マスタから「夏期」表記を柔軟に判別して残コマを抽出
                        contract_remains = {} 
                        for _, row in df_contracts.iterrows():
                            c_name = str(row.get("講習名", ""))
                            # 表記揺れ（「夏期講習」「2026夏期」など）に部分一致で対応
                        ...
                        if not df_lessons.empty and "日付" in df_lessons.columns:
                            ...
                            for _, row in df_past.iterrows():
                                ...
                                sn_c = str(row.get("生徒名", "")).replace(" ", "").replace(" ", "").strip()
                                tn_c = str(row.get("講師名", "")).replace(" ", "").replace(" ", "").strip()
                        ...
                        if new_lessons:
                            ...
                            st.success("🎉 自動コマ組みが完了しました！下のプレビューで配置を確認して確定してください。")
                        else:
                            st.warning("⚠️ 新しく割り当てられる契約コマが見つかりませんでした。")

                if f"new_lessons_{is_summer}" in st.session_state:
                    st.subheader("📋 【下書き】自動生成された授業予定表")
                    df_new_flat = pd.DataFrame(st.session_state[f"new_lessons_{is_summer}"])
                    ...
                    components.html(f"""
                        ...
                        function downloadPDF() {{
                            ...
                            setTimeout(() => {{
                                ...
                                style.innerHTML = `
                                    .print-optimized-table {{ width: 100%; border-collapse: collapse; font-family: sans-serif; font-size: 11px; }}
                                    ...
                                    .student-badge {{ font-size: 10px; font-weight: bold; padding: 3px; margin: 1px 0; border-radius: 2px; border: 1px solid rgba(0,0,0,0.1); }}
                        ...
                        if view_tab_labels:
                            ...
                            for idx, w_dates in enumerate(view_weeks[:4]):
                                with view_tabs[idx]:
                                    ...
                                    # PDF用に一意のクラス名(print-container-True/False)を割り当て
