...
def render_line_report_page():
    ...
    # 🌟 校舎ごとにタブで表示
    tabs = st.tabs([f"🏫 {k} ({len(v)}名)" for k, v in data_buckets.items() if len(v) > 0 or k != "その他"])

    for t_idx, (bucket_name, students) in enumerate(data_buckets.items()):
        with tabs[t_idx]:
            if not students:
