...
def render_quiz_image_manager_page():
    ...
    if uploaded_files:
        ...
        with st.container(border=True):
            ...
            # 🌟 変更点：日付選択カレンダーを追加するため、3つのカラムに分割！
            c_meta1, c_meta2, c_meta3 = st.columns([1.2, 1.2, 1.6])
            
            # 💡 日付のデフォルト値を今日に設定
            file_date = c_meta1.date_input("📅 記録する日付", datetime.date.today(), key=f"meta_date_{student_id}")
            subj = c_meta2.selectbox("教科", ["英語", "数学", "国語", "理科", "社会", "その他"], key=f"meta_sub_{student_id}")
            title_suffix = c_meta3.text_input("補足名 (任意)", placeholder="例: 二次関数", key=f"meta_title_{student_id}")
            ...
            if st.button("🚀 この設定でGoogle Driveへ一括保存する", type="primary", use_container_width=True):
                ...
                success_count = 0
                # 🌟 変更点：カレンダーで選んだ日付を「YYYYMMDD」の文字列に変換
                ...
                for i, u_file in enumerate(uploaded_files):
                    ...
                    # 🌟 変更点：今日の日付ではなく、選んだ日付（date_str）がファイル名の頭につく！
                    file_name = f"{date_str}_{subj}{suffix_str}{seq_str}.jpg"
